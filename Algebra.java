...
public class Algebra {
	...
	public static int mod(int x1, int x2) {
		...
		if (x1 < 0) {
			if (x2 < 0) {
				...
				int tempX2 = times(x2, -1);
				while (module  < tempX2) {
					module = minus(module, tempX2);
				}
				module = plus(x2, module);
			}
		} else {
			if (x2 < 0) {
				module = times(module, -1);
				while (module < x2) {
					module = minus(module, x2);
				}
				module = times(plus(times(x2, -1), module), -1);
	...
	public static int sqrt(int x) {
		for (int i = 1; i <= x; i++){
			// not much to tell since x = s^2 than if we power by 2 all the numbers from 1 to s, lets call them i, by some point x will be lower than i^2, therefore the closest number was the one b
			if (pow(i, 2) > x){
				return i - 1;
			}
		}
